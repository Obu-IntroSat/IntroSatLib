...
namespace IntroSatLib
{
	namespace Filters
	{
		...
		class ScalableFilter: public IntroSatLib::Base::BaseFilter<T>
		{
			...
			public:
				ScalableFilter(IntroSatLib::Base::BaseFilter<T1>& innerFilter, int8_t maxScale): _innerFilter(innerFilter)
				{
					_minScale = 0;
					_maxScale = maxScale;
				}

				ScalableFilter(IntroSatLib::Base::BaseFilter<T1>& innerFilter, int8_t minScale, int8_t maxScale): _innerFilter(innerFilter)
				{
					_minScale = minScale;
					_maxScale = maxScale;
				}
