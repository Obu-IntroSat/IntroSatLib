...
#ifndef ARDUINO
BaseFlyWheel::BaseFlyWheel(I2C_HandleTypeDef *hi2c, uint8_t address) :
...
void BaseFlyWheel::MaxAbsSpeed(uint16_t maxAbsSpeed)
{
	if (_channel == 1)
	{
		_i2c.write(RegisterMap::Need_F_ChannelSpeed, (uint8_t*) &maxAbsSpeed,
				2);
	}
	if (_channel == 2)
	{
		_i2c.write(RegisterMap::Need_S_ChannelSpeed, (uint8_t*) &maxAbsSpeed,
				2);
	}
}
...
BaseFlyWheel::BaseFlyWheel(BaseFlyWheel &&other) :
		BaseDevice(other)
{
	_version = other._version;
	_channel = other._channel;
